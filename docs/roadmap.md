...
| Proxy: TLS, SNI, static, caching, LB, live routing reload, EWS/ActiveSync/Autodiscover | `crates/proxy` | 160 | path traversal, Range, 304, health hysteresis; a config edit hot-reloads the routing table with no dropped connection; EWS/ActiveSync Basic-auth gated and only served from `autodiscover.<domain>` |
...
| Mail: SMTP, submission, DKIM, outbound, IMAP, EWS, ActiveSync (WBXML), Autodiscover | `crates/mail` | 225 | open-relay defence, credential protection, framing; a message sent by raw SMTP was read back out of the Maildir; IMAP LOGIN/SELECT/FETCH/STORE walked end-to-end over a live connection; EWS `GetItem`/ActiveSync `Fetch` serve the same store as raw MIME, `CreateItem`/`SendMail` route through the same local/outbound split submission uses |
