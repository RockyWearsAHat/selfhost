...
**Convention followed, not invented.** This is a config-reload-requires-restart subsystem, exactly like `[desktop]`, `[dns]`, and `[[shares]]`: editing `enabled` or a relay's addresses takes effect at the daemon's *next restart*. Devices are not config: a Peer exists only because a signed-in account enrolled it (named `<person>-<device>` by the server, listed in `<key_dir>/roster`, bound to its Person in `<data_dir>/vpn.peers`), is admitted at the next handshake, and is removed with `selfhost people forget-device <person> <device>`. `[[vpn.peers]]` is refused by `selfhost check`. There is no live reconcile-now path for `[[vpn]]`, and none was added.
