...
impl Config {
    ...
    /// The VPN relays, when any are declared, must be individually well-formed,
    /// must not collide with one another, and must not be pointed at this
    /// deployment's own control plane.
    ///
    ...
    /// for exactly that reason (`docs/SECURITY.md` §2, "admin_bind is unchanged
    /// and is not negotiable"). A relay in front of it would put the root
    /// credential's door on a socket off this machine, behind nothing but a VPN
    /// key — and `docs/labs/vpn-identity-lab.dx` is the record of what a VPN key
    /// is worth as an authorisation. It is a refusal rather than a warning
    /// because a deployment that logs a line nobody reads is a deployment serving
    /// its own control plane to the tunnel.
    fn check_vpn(&self, problems: &mut Vec<Problem>) {
        crate::vpn::check_relays(&self.vpn, "vpn", problems);

        // Parsed rather than compared as text: "127.0.0.1:9191" and
        // "127.000.000.001:9191" are the same socket and a text comparison would
        // let the second one through the rule that matters most here.
        let admin = self.server.admin_bind.parse::<SocketAddr>().ok();
        let bound: Vec<(&str, Option<SocketAddr>)> = vec![
            ("server.http_bind", self.server.http_bind.parse().ok()),
            ("server.https_bind", self.server.https_bind.parse().ok()),
            ("server.admin_bind", admin),
        ];

        for (i, relay) in self.vpn.iter().enumerate() {
            if let Some(listen) = relay.listen_addr() {
                for (field, existing) in &bound {
                    // `collides`, not equality: the proxy's 0.0.0.0:443 and a
                    // relay's 127.0.0.1:443 are different addresses and only one
                    // of them can be bound.
                    if existing.is_some_and(|existing| crate::vpn::collides(existing, listen)) {
                        problems.push(Problem {
                            field: format!("vpn[{i}].listen"),
                            message: format!(
                                "\"{}\" is already {field}. Whichever of the two starts second \
                                 fails to bind, and which one that is depends on start order — \
                                 so this deployment would answer that port with the proxy or \
                                 with a tunnel depending on how it was last restarted.",
                                relay.listen
                            ),
                        });
                    }
...
pub fn console_gate_refusal(entry: &str) -> Option<String> {
    let (address, prefix) = cidr_parts(entry)?;
...
///
/// Shared with [`crate::vpn`], which asks the same two questions of a relay's
/// subnet — is it private, and how broad is it. A second splitter would be a
/// second answer to "what does /24 mean here", which is exactly the shape of
/// divergence this crate avoids everywhere else.
pub(crate) fn cidr_parts(entry: &str) -> Option<(IpAddr, u8)> {
...
mod tests {
    ...
    fn with_relays(relays: Vec<crate::vpn::Relay>) -> Config {
        let mut config = config(vec![owner_node()], vec![site("a", "example.com")]);
        config.vpn = relays;
        config
    }

    #[test]
    fn a_declared_relay_validates_as_part_of_the_whole_document() {
        assert!(with_relays(vec![relay()]).validate().is_ok());
    }

    #[test]
    fn a_relay_may_not_be_put_in_front_of_the_admin_api() {
        // The sharpest refusal in this file: the control API answers the bearer
        // token as the owner, and binds loopback so nothing off this box reaches
        // it. A relay in front of it is that credential on a public socket.
        let mut fronting_control = relay();
        fronting_control.forward = "127.0.0.1:9191".into();
        let problems = problems_of(&with_relays(vec![fronting_control]));
        assert!(problems.iter().any(|p| p.field == "vpn[0].forward"), "{problems:?}");
        assert!(
            problems[0].message.contains("admin_bind"),
            "the refusal must name what was pointed at: {problems:?}"
        );
    }

    #[test]
    fn a_relay_may_not_claim_a_socket_the_deployment_already_binds() {
        for (listen, _) in [("0.0.0.0:80", ()), ("0.0.0.0:443", ()), ("127.0.0.1:9191", ())] {
            let mut collides = relay();
            collides.listen = listen.into();
            // The admin bind is loopback, so that one is not "public" — set the
            // flag to match, or the honesty rule reports instead of this one.
            collides.public = !listen.starts_with("127.");
            let problems = problems_of(&with_relays(vec![collides]));
            assert!(problems.iter().any(|p| p.field == "vpn[0].listen"), "{listen}: {problems:?}");
        }
    }
