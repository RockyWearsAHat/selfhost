...
impl Capability {
    ...
    /// routes exist.
    ///
    /// `site.admin` is the day this promise was kept: `crates/app/admin::site_api`
    /// now has real routes behind it (`GET/POST/DELETE /api/sites`, the
    /// add-domain/remove-domain pair, and the site-scoped file routes), so this
    /// is the one line that changed. `dns.admin` and `mail.admin` remain
    /// promises — nothing consumes them yet.
    ///
    ...
    pub fn is_honoured(&self) -> bool {
        match self {
            ...
            Self::DnsAdmin | Self::MailAdmin => false,
        }
    }
