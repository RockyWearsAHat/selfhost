...
use selfhost_identity::{Capability, Grants, Identity, NodeName, Opening, People, PersonName, ShareId};
...
const SURFACE: &[(&str, &str, Reach)] = &[
    ...
    // Demand::Authenticated again — the wall only asks whether this caller is
    // known at all; whether they hold `vpn.access:<location>` for the body's
    // own location is a question the handler asks Policy itself, not the wall
    ...
    // dispatch (`Api::handle`, before the route-specific match), so this holds
    // whether or not this deployment was even built `.with_site_admin(..)` —
    // an un-wired site subsystem's own 404 is never reachable by a caller who
    // was refused first.
    ("GET", "/api/sites", Reach::Withheld),
    ("GET", "/api/sites/anything", Reach::Withheld),
    ("POST", "/api/sites", Reach::Withheld),
...
async fn a_capability_no_route_honours_cannot_be_granted() {
    // `dns.admin` and `mail.admin` are real words in a real vocabulary that no
    // handler asks for. Granting one wrote a row an operator reads as "she can
    // manage the DNS" with no code path that agrees — a promise, and the worst
    // kind, because the failure surfaces as nothing happening at all rather
    // than as an error. `site.admin` used to be a third word in this list; it
    // has a real route now (`crates/app/admin::site_api`) and belongs in
    // `site_admin_is_a_kept_promise_and_may_be_granted` instead.
    ...
    let ungrantable: Vec<&str> = listed
        ...
        .collect();
    assert_eq!(ungrantable, vec!["dns.admin", "mail.admin"]);
}

#[tokio::test]
async fn site_admin_is_a_kept_promise_and_may_be_granted() {
    // The other half of the story above: `site.admin` used to belong in
    // `a_capability_no_route_honours_cannot_be_granted`'s list and now belongs
    // here instead, because `crates/app/admin::site_api` gave it a real route
    // — granting it succeeds, and it is no longer offered as ungrantable.
    let (api, dir, _sessions) = deployment_with_sessions("kept-promise").await;
    let owner_cookie = password_login(&api).await;
    let headers = [("Cookie", owner_cookie.as_str()), ("X-Selfhost-Console", "1")];
    ...
    let accepted = call_with(
        ...
        &format!("/api/people/{NEWCOMER}"),
        &headers,
        r#"{"grants":["console.read","site.admin"]}"#,
    )
    .await;
    assert_eq!(accepted.status.code(), 200, "site.admin is a kept promise");
    assert!(
        People::load(dir.path())
            .grants_for(&Identity::Person(PersonName::parse(NEWCOMER).unwrap()))
            .holds(&Capability::SiteAdmin),
...
async fn no_unhonoured_word_is_named_by_any_routes_demand() {
    ...
    let demands = source
        ...
        .0;
    // `site.admin` is deliberately absent here now: `crates/app/admin::site_api`
    // gave it a real route, so `Route::demand` legitimately names
    // `Capability::SiteAdmin` and `Capability::is_honoured` agrees — the
    // property this test polices no longer applies to that word.
    for (variant, word) in [("DnsAdmin", "dns.admin"), ("MailAdmin", "mail.admin")] {
        assert!(
