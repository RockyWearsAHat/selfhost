...
use selfhost_identity::{Capability, Grants, Opening, PassKey, People, PersonName, SiteName};
...
fn deployment(name: &str) -> Deployment {
    ...
    let config = selfhost_config::Config::parse(CONFIG).expect("a valid config");
