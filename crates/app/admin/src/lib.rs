...
//! (`allowed_cidrs`, in practice the Secure-VPN subnet `10.66.0.0/24`) has
...
enum Demand {
    ...
    ///
    /// Split out of [`Demand::OwnerOnly`] because it is the one act that must
    /// stay open to the console password for ever, and the reason is the reason
    /// this whole model is safe: a passkey is a thing that can be lost, and the
    /// only door left when the last one is in a drawer is the password. A
    /// password login may therefore always enrol *another* passkey — and may
    /// never remove one, which is what stops "enrol a replacement" from being
    /// "delete every credential that outranks me and take the box back".
    ///
    /// Closed to the bearer token: a WebAuthn ceremony needs hardware and a
    /// person, `crates/ui/console` never calls these routes, and a credential
    /// living in a file should not be able to mint a person who will still be
    /// here after it is rotated.
    Enrolment,
    /// Reading what only the deployment itself may read: the passkey roster and
    /// the audit trail.
    ///
    /// Neither is a capability, and deliberately: there is no word for "may read
    /// the record of everyone else", and a person who could be granted it could
    /// watch the record of their own supervision. But both are read by the native
    /// console over its SSH tunnel, so this is the one authority-shaped demand
    /// the machine credential satisfies — it is reading the box's own state, not
    /// changing anybody's standing in it.
    OwnerReads,
...
impl<'a> Route<'a> {
    ...
    fn demand(&self) -> Demand {
        match self {
            ...
            // they did with it, and the roster names everyone who can prove they
            // are somebody; reading either is not one of the powers a grant
            // confers, and there is no capability that honestly describes "may
            // read the record of everyone else". A person who could be granted
            // it could watch the record of their own supervision.
            Self::AuditTrail | Self::ListPasskeys => Demand::OwnerReads,
            // Enrolment stays open to the console password for ever; removal
            // never does. The pair is the recovery path and its one guard rail:
            // lose a device and the password can enrol its replacement, and a
            // password that could instead *delete* the enrolled passkeys would be
            // a shared secret that can undo its own demotion.
            Self::RegisterChallenge | Self::Register => Demand::Enrolment,
...
/// The identity behind the console password. Passkey sessions carry the
/// holder's own name instead, and the bearer token carries the machine's.
///
/// It stopped covering the bearer token when the token stopped being the
/// operator — see [`Identity::Machine`](selfhost_identity::Identity::Machine).
/// The name a password login mints is unchanged, and deliberately: what changed
/// is what that identity may do once this deployment has a credential naming a
/// person, and that is decided in the policy rather than by relabelling a
/// session.
...
impl Api {
    ...
    /// permits.
    ///
    /// What is stored here is the *settings* half of the policy. The other half
    /// is a fact that changes while the daemon runs — whether this deployment has
    /// a passkey enrolled — and that is joined on in [`Api::policy`] rather than
    /// here, because a value captured at start-up would still be calling the
    /// console password a root credential an hour after the first registration.
    pub fn with_policy(mut self, policy: Policy) -> Self {
        self.policy = policy;
        self
    }

    /// The authorisation policy as it stands for *this* request.
    ///
    /// The stored settings, plus the one fact the decision needs that nobody can
    /// write in a file: whether a credential naming a person exists here yet.
    /// Every capability decision in this crate goes through this rather than
    /// through the stored field, so a registration that finishes at 11:03 has
    /// demoted the console password by 11:03 and not at the next restart.
    fn policy(&self) -> Policy {
        self.policy.with_passkey_enrolled(self.a_passkey_is_enrolled())
    }

    /// Whether this deployment has at least one enrolled passkey.
    ///
    /// The pivot of the whole identity model, so it is worth being exact about
    /// what `false` covers: no passkeys registered, *and also* no passkey support
    /// wired at all — a daemon with no console site has no relying-party
    /// identity, so it can never enrol one. Both answer `false`, and both mean
    /// the same thing for authorisation: there is no credential on this box that
    /// names a person, so the console password is the only way in and demoting it
    /// would lock the operator out of a deployment that has done nothing wrong.
    fn a_passkey_is_enrolled(&self) -> bool {
        self.webauthn().is_some_and(|(_, webauthn)| !webauthn.is_empty())
    }
    ...
    fn vpn_forget_device(&self, caller: &Caller, peer: &str) -> Response {
        ...
        if vpn.owner_of_peer(peer).as_deref() != Some(name.as_str()) {
            return problem(Status(404), "not one of your devices");
        }
    ...
    pub fn caller(&self, request: &Request) -> Option<Caller> {
        if self.bearer_authorised(request) {
            // The bearer token is the box's own credential, and the caller it
            // builds says so: `Identity::Machine`, holding the machine's fixed
            // list rather than the operator's blanket authority. It used to
    ...
    async fn login_owner(&self, console: &ConsoleAuth, password: &str) -> Response {
        ...
        console.gate.reset();
        // The session this mints is the deployment's owner, whatever device
        // typed it — and how much that is worth is decided per request rather
