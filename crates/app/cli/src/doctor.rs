...
// Not a copy: the health probes and the authoritative-DNS section below have to
// reach a wildcard-bound server the same way, or one of them reports a component
// dead that the other finds perfectly well.
use crate::health::loopback_target;
...
pub async fn run(config: &Config, project_dir: &Path, deep: bool, scan_lan: bool) -> Report {
    ...
    check_serving(&mut report, config, project_dir).await;
    check_supervision(&mut report, config, project_dir);
...
fn check_secrets(report: &mut Report, config: &Config, project_dir: &Path) {
    ...
    section.checks.push(named_identity_check(config, &data_dir));
}

/// Whether anything on this deployment can say *which person* acted.
///
/// The condition this reports is real, temporary and self-clearing, and it is
/// reported precisely because it is easy to stay in for ever without noticing.
/// Until one passkey is enrolled, the console password is the deployment's root
/// credential: it names nobody, it is the same secret for everybody who is ever
/// given it, and every line it writes in the audit trail says `owner`. The
/// capability model behind the console is real and carefully tested, and in that
/// state it is being enforced against one identity that everything resolves to,
/// which is not enforcement.
///
/// It is a [`Verdict::Warn`] rather than a [`Verdict::Fail`] because a box in
/// this state is not broken — it is a box somebody has just installed, and its
/// one credential is the only way in, which is exactly why the demotion in
/// `crates/identity` waits for a passkey rather than for a switch. A `Fail`
/// would make `selfhost doctor` exit non-zero on every fresh install and teach
/// operators to ignore the line that matters.
///
/// The verdict is derived from the passkey store itself rather than from
/// configuration, so it reports what the daemon would actually decide: the same
/// `Passkeys::is_empty` the API asks on every request.
fn named_identity_check(config: &Config, data_dir: &Path) -> Check {
    let holders: Vec<String> = selfhost_admin::webauthn::Passkeys::load(data_dir)
        .list()
        .into_iter()
        .map(|passkey| passkey.user)
        .collect();
    let can_enrol = config.sites.iter().any(|site| site.console && !site.domains.is_empty());
    named_identity_verdict(&holders, can_enrol)
}

/// The rule behind [`named_identity_check`], given the observation.
///
/// Split out on the same grounds as [`permission_check`]: the interesting part
/// is which of three sentences an operator is handed, and it should be asserted
/// directly rather than by arranging a passkey file on disk.
fn named_identity_verdict(holders: &[String], can_enrol: bool) -> Check {
    if !holders.is_empty() {
        return Check::new(
            "named identity",
            Verdict::Pass,
            format!(
                "{} passkey(s) enrolled ({}), so the console password is no longer a root \
                 credential: it reads the console and enrols another passkey, and every act \
                 that changes this machine needs a credential that names whoever performed it. \
                 The bearer token is the identity \"machine\" and holds the machine's list, not \
                 the owner's.",
                holders.len(),
                holders.join(", ")
            ),
    ...
    let detail = "no passkey is enrolled, so the console password is still this deployment's \
                  root credential. It names nobody: everyone who is ever given it is the same \
                  identity, the capability model behind the console has one identity to enforce \
                  against, and the audit trail cannot answer which human acted.";
    if !can_enrol {
        return Check::new(
            "named identity",
            Verdict::Warn,
            format!(
                "{detail} No site sets console = true with a domain, so there is no WebAuthn \
                 relying party and no passkey can be enrolled here at all — this state is \
                 permanent until that changes."
            ),
        )
        .with_fix(
            "Mark the console's site `console = true` and give it a domain, restart the daemon, \
             then enrol a passkey from a browser at that hostname.",
        );
    }
    Check::new("named identity", Verdict::Warn, detail.to_owned()).with_fix(
        "Log in to the console and register a passkey. From that moment the password is a \
         bootstrap and recovery credential only — it still logs in, still reads the console and \
         can still enrol a replacement passkey if a device is lost, and it can no longer control \
         a service, open a share or manage people.",
    )
}
...
/// Whether each part of the deployment is actually **serving**, as opposed to
/// running.
///
/// The section that would have caught the 2026-08-16 outage. [`check_binds`]
/// above asks whether a port *could* be bound and reads "already in use" as
/// reassurance — which it usually is, and which on that day it also was, for the
/// two ports that were fine. Nothing asked the nameserver a question. This does,
/// and it uses [`crate::health`] rather than its own client so `doctor` and the
/// daemon's convergence loop cannot come to hold different opinions about what
/// "up" means.
async fn check_serving(report: &mut Report, config: &Config, project_dir: &Path) {
    let probes = crate::health::probe_all(config, project_dir).await;
    let section = report.section("Serving");
    for probe in probes {
        let verdict = match probe.serving {
            crate::health::Serving::Yes => Verdict::Pass,
            crate::health::Serving::No => Verdict::Fail,
            crate::health::Serving::NotDeclared => Verdict::Skipped,
            crate::health::Serving::Untestable => Verdict::Unknown,
        };
        let mut check = Check::new(
            format!("{} — {}", probe.component.label(), probe.target),
            verdict,
            probe.detail,
        );
        if verdict == Verdict::Fail {
            check = check.with_fix(
                "This deployment is running and this part of it is not answering. That is the \
                 shape of the outage this section was written for: a task in the correct state \
                 whose service is not serving. The daemon's convergence loop repairs what it \
                 can and records every attempt in the repair ledger — see the Supervision \
                 section below, and `selfhost service check` for whether the registration \
                 itself is what it was installed as.",
            );
        }
...
/// What self-repair has had to do, and what it has given up on.
///
/// Reads [`crate::converge::Ledger`] rather than measuring anything. It is here
/// because an escalation is the one state the probes above cannot show: a
/// component the daemon repaired three times and then stopped repairing may be
/// serving perfectly at the moment `doctor` runs and still be the most important
/// thing on the page.
fn check_supervision(report: &mut Report, config: &Config, project_dir: &Path) {
    let data_dir = crate::teardown::data_dir(config, project_dir);
    let ledger = crate::converge::Ledger::in_dir(&data_dir);
    let entries = ledger.entries();
    let section = report.section("Supervision");
    ...
    if entries.is_empty() {
        section.checks.push(Check::new(
            ...
            Verdict::Pass,
            format!(
                "nothing recorded in {} — this deployment has never had to repair itself",
                ledger.path().display()
            ),
        ));
        return;
    ...
    if outstanding.is_empty() {
        section.checks.push(Check::new(
            ...
            Verdict::Warn,
            format!(
                "{} entr(ies) in {}, none outstanding — something has broken and been repaired",
                entries.len(),
                ledger.path().display()
            ),
        ));
    ...
    for entry in outstanding {
        section.checks.push(
            Check::new(
                format!("supervision gave up on {}", entry.component),
                Verdict::Fail,
                entry.detail,
            )
            .with_fix(format!(
                "Supervision stopped repairing this deliberately: something restarted three \
                 times without coming back is a fault to look at, not a thing to restart a \
                 fourth time. Read {} for the whole sequence. Once the cause is fixed, the \
                 next successful probe clears this by itself — nothing needs restarting to \
                 clear it.",
                ledger.path().display()
            )),
        );
...
mod tests {
    ...
    #[test]
    fn a_deployment_with_no_passkey_is_told_its_password_is_still_root() {
        // The finding that keeps the temporary state from becoming the permanent
        // one. It is a warning and not a failure on purpose: a box in this state
        // is a box somebody has just installed, and a `FAIL` on every fresh
        // install teaches operators to ignore the line that matters.
        let check = named_identity_verdict(&[], true);
        assert_eq!(check.verdict, Verdict::Warn);
        assert!(check.detail.contains("root credential"), "{}", check.detail);
        assert!(check.detail.contains("names nobody"), "{}", check.detail);
        assert!(
            check.detail.contains("which human acted"),
            "the audit consequence is the half an operator has to hear: {}",
            check.detail
        );
        let fix = check.fix.expect("a finding without a fix is half a diagnostic");
        assert!(fix.contains("register a passkey"), "{fix}");
        // And the fix has to say what it costs, or nobody does it: the password
        // keeps working, which is the whole reason this is safe to act on.
        assert!(fix.contains("replacement passkey"), "{fix}");
    }

    #[test]
    fn a_deployment_that_cannot_enrol_a_passkey_is_told_the_state_is_permanent() {
        // Same warning, different fix. Without a console site there is no
        // WebAuthn relying party, so "register a passkey" is advice nobody can
        // follow and the operator would be left rereading it.
        let check = named_identity_verdict(&[], false);
        assert_eq!(check.verdict, Verdict::Warn);
        assert!(check.detail.contains("relying party"), "{}", check.detail);
        assert!(check.detail.contains("permanent"), "{}", check.detail);
        assert!(check.fix.expect("a fix").contains("console = true"));
    }

    #[test]
    fn a_deployment_with_a_named_credential_passes_and_says_what_changed() {
        let check = named_identity_verdict(&["owner".to_owned(), "Mom".to_owned()], true);
        assert_eq!(check.verdict, Verdict::Pass);
        assert!(check.detail.contains("2 passkey(s)"), "{}", check.detail);
        assert!(check.detail.contains("Mom"), "the roster is named: {}", check.detail);
        assert!(
            check.detail.contains("no longer a root credential"),
            "a pass here is a statement about the password, not about passkeys: {}",
            check.detail
        );
        assert!(check.fix.is_none(), "nothing to do");
    }
