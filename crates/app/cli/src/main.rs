...
Usage
  ...
  selfhost --remote <host> <command>

Options
  --remote <host>            Ask another machine instead of this one, over HTTPS
                             through its console site's /api relay. Read-only:
                             `services`, `app list` and `app show <name>` are
                             carried; every other command refuses and exits
                             non-zero rather than quietly acting here. The bearer
                             token comes from SELFHOST_TOKEN or ~/.selfhost/token
                             — never from an argument, where `ps` and the shell
...
Commands
  ...
  health                     Ask each part of this deployment the question it
                             exists to answer — does DNS resolve, does the proxy
                             answer on 80 and 443, does the control API answer on
                             loopback — and print what came back. A live process
                             proves nothing; this is what the daemon's own
  ...
  app <list|show|deploy>     Applications deployed from Git repositories.
                             `deploy <name>` asks the running daemon to deploy
  ...
  service <install|uninstall|status|check> [--system] [--yes] [--repair]
                             ...
                             difference; `check --repair` rewrites the settings
                             back, leaving what the task *runs* untouched. This
...
fn main() -> ExitCode {
    // `--remote` is taken out of the arguments before anything else looks at
    // them, because it decides *which machine* every command below is about —
    // and because a command that has no remote implementation has to be able to
    // refuse. A flag left in the vector would be seen by a local command as an
    // argument it did not recognise, and it would carry on here: the operator
    // named another machine and this one would be the one acted on. See
    // [`remote_client`] for why that is the failure this is shaped around.
    let (arguments, remote) = match remote_client::split(std::env::args().skip(1).collect()) {
        Ok(split) => split,
        Err(message) => {
            eprintln!("\n✗ {message}");
    ...
    // Nothing below this line runs when another machine was named: the remote
    // routing is a closed set of read-only commands and everything outside it is
    // an error, never a local fallback.
    if let Some(remote) = &remote {
        return finish(remote_client::run(&arguments, remote));
    }
...
/// Turns a command's outcome into this process's exit status.
///
/// One place rather than two, so the remote path and the local one cannot come
/// to differ about what failure looks like: a non-zero exit and the reason on
/// stderr, which is what a script reading this binary depends on.
...
/// Asks every part of the deployment whether it is serving, and prints the
/// answers.
///
/// Separate from `doctor` even though `doctor` includes the same section, because
/// they answer different questions and are run at different times. `doctor` is a
/// long diagnostic somebody reads once when something is wrong; this is four
/// loopback round trips a monitor can run every minute. Both call
/// [`health::probe_all`], so neither can be right while the other is wrong.
///
/// Exits non-zero only for a component that is *running and not serving*. A
/// development tree where nothing is up reports untestable and exits zero: a
/// health check that fails on a laptop is one people stop running.
fn health_command() -> Result<(), String> {
    let (config, project_dir) = load()?;
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
    ...
    let probes = runtime.block_on(health::probe_all(&config, &project_dir));
    let width = probes.iter().map(|p| p.component.label().len()).max().unwrap_or(0);
    let mut faults = 0;
    for probe in &probes {
        let marker = match probe.serving {
            health::Serving::Yes => "serving",
            health::Serving::No => "NOT SERVING",
            health::Serving::NotDeclared => "not declared",
            health::Serving::Untestable => "untestable",
        };
        if probe.serving.is_fault() {
            faults += 1;
        }
        println!("  {:<width$}  {marker}", probe.component.label());
        println!("  {:<width$}    asked   {}", "", probe.target);
        println!("  {:<width$}    got     {}", "", probe.detail);
    }
...
async fn serve_everything(
    ...
    let outcome = tokio::select! {
        ...
        // Asks every part of this deployment whether it is *serving* — not
        // whether it is running — and repairs, within a bound, what is not.
        // Here rather than in a registration of its own on purpose: a watchdog
        // that is a separate service is one more thing that can die quietly
        // while everything looks fine, which is the exact failure it exists to
        // catch. Never returns, so it occupies an arm without firing.
        _ = converge::converge_forever(
            config.clone(),
            project_dir.clone(),
            data_dir.clone(),
        ) => Ok(()),
...
fn service_command(arguments: &[String]) -> Result<(), String> {
    ...
    match arguments.get(1).map(String::as_str) {
        ...
        Some("check") => {
            service_check_command(system, arguments.iter().any(|argument| argument == "--repair"))
        }
        Some(other) => Err(format!(
            "unknown service subcommand \"{other}\" — expected install, uninstall, status, or \
             check\n\n{USAGE}"
        )),
        None => Err(format!(
            "service needs a subcommand: install, uninstall, status, or check\n\n{USAGE}"
        )),
    }
}

/// Compares every installed registration against what it was meant to be, and
/// optionally puts it back.
///
/// # Why this command exists
///
/// `selfhost service install` has always emitted correct task XML. The
/// registration that took the deployment down was created by a different path
/// and carried Windows' defaults — a 72-hour execution limit and no restart
/// policy — and nothing ever compared the two. An installer being right is not
/// the same as an installation being right, and only the second keeps a box up.
///
/// Dry by default and non-zero when anything has drifted, so it is usable as a
/// gate in a script. `--repair` rewrites the settings and leaves the action, the
/// principal and the triggers exactly as they were: this command has no business
/// deciding what `selfhost-vpn` runs.
fn service_check_command(system: bool, repair: bool) -> Result<(), String> {
    let audits = service_install::audit_installed(system);
    if audits.is_empty() {
        return Err(service_install::unsupported_message());
    }

    let mut drifted = 0;
    let mut unrepaired = 0;
    for audit in &audits {
        if let Some(reason) = &audit.untestable {
            println!("  ????  {:<20} {reason}", audit.label);
            unrepaired += 1;
            continue;
        }
        if !audit.present {
            println!("  ----  {:<20} not registered on this machine", audit.label);
            continue;
        }
        if audit.faults.is_empty() {
            println!("  ok    {:<20} matches what it was installed as", audit.label);
            continue;
        }
        drifted += 1;
        println!("  DRIFT {:<20} {} difference(s):", audit.label, audit.faults.len());
        for fault in &audit.faults {
            println!("          {fault}");
        }
        if repair {
            match service_install::repair_settings(&audit.label) {
                Ok(true) => println!("        repaired — the settings now match; the action is unchanged"),
                Ok(false) => println!("        nothing to write"),
                Err(error) => {
                    println!("        NOT repaired: {error}");
                    unrepaired += 1;
                }
            }
        }
    ...
    println!();
    match (drifted, repair, unrepaired) {
        (0, _, 0) => {
            println!("Every registration matches what it was installed as.");
            Ok(())
        }
        (_, true, 0) => {
            println!("{drifted} registration(s) had drifted and were repaired.");
            Ok(())
        }
        (_, false, _) => Err(format!(
            "{drifted} registration(s) do not match what they were installed as. \
             Re-run with --repair to put the settings back."
        )),
        (_, true, _) => {
            Err(format!("{unrepaired} registration(s) could not be checked or repaired"))
        }
    }
}
