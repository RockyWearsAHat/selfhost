...
pub struct Viewer {
    ...
    pub owner: bool,
    /// Whether this is the box's own bearer token, whose authority is likewise
    /// an identity rather than a grant list — a fixed list in the daemon's
    /// policy, not something the registry can widen.
    ///
    /// This console is the credential's usual holder: it reaches the daemon over
    /// an SSH tunnel with the token and no cookie, so on a normally-configured
    /// deployment this is the flag that is set and `owner` is not. Reading only
    /// `owner` would draw an empty tab rail here, which is a console that looks
    /// like it has lost its screens rather than one reporting a refusal.
    pub machine: bool,
...
impl Viewer {
    ...
    pub fn from_json(value: &Json) -> Option<Self> {
        Some(Self {
            ...
            // Optional, unlike `owner`: a daemon older than the identity split
            // does not send it, and the honest reading of its absence is "not
            // the machine" rather than a viewer this console refuses to parse.
            machine: value.get("machine").and_then(Json::as_bool).unwrap_or(false),
    ...
    pub fn may_open(&self, screen: Screen) -> bool {
        // Both identities whose authority is an identity rather than a grant
        // list. The machine's list covers every route this console calls — the
        // services, the shares, a screen, the passkey roster and the audit
        // trail — so every tab has something behind it. The one thing it may
        // not do is revoke a passkey, and that button reports its own refusal,
        // which is the right place for it: hiding a whole screen to hide one
        // button is how a console starts lying about what is there.
        if self.owner || self.machine {
            return true;
        }
        match screen {
...
mod tests {
    ...
    #[test]
    fn the_machine_holds_every_screen_this_console_can_reach() {
        // This console's own credential. The bearer token is no longer the
        // owner, and a tab rail keyed only on `owner` would come up empty on
        // exactly the deployment this program is written for — an empty rail
        // reads as a console that has lost its screens, not as one refusing.
        let machine = Viewer { machine: true, name: "machine".into(), ..holding(&[]) };
        assert!(!machine.owner, "the token is not the operator");
        assert_eq!(Screen::for_viewer(Some(&machine)), Screen::ALL.to_vec());
    }
    ...
    fn a_whoami_answer_is_read_from_the_wire() {
        ...
        assert!(!viewer.owner);
        assert!(!viewer.machine, "an absent flag reads as \"not the machine\", not as a refusal");
        assert_eq!(viewer.grants, ["console.read"]);

        // And the answer this console usually gets, over its own tunnel.
        let token = selfhost_json::parse(
            r#"{"name":"machine","owner":false,"machine":true,"credential":"bearer","grants":[]}"#,
        )
        .expect("legal JSON");
        let viewer = Viewer::from_json(&token).expect("a viewer");
        assert!(viewer.machine);
        assert!(!viewer.owner);
        assert_eq!(viewer.credential, "bearer");
