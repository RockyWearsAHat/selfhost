...
pub struct Wiring<'a> {
    ...
    /// The authorisation model. Consulted even though the only credential that
    /// reaches here is the owner's: a check that is skipped because it *happens*
    /// to be redundant is a check nobody notices has become load-bearing.
    pub policy: &'a Policy,
