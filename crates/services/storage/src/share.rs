...
impl Share {
    ...
    ///
    /// A caller with no matching grant is refused, with two exceptions that are
    ...
    /// themselves out of their own storage by editing a grant list. The box's
    /// own token holds [`Mode::Write`] and stops there — it may move the bytes
    /// the native console asks it to move, and it may not change the share
    /// itself. That is the same line the capability model draws for it: it holds
    /// `files.read` and `files.write` and not `files.admin`, and the two must
    /// agree, or the answer to "may this token administer a share" would depend
    /// on which of the two layers happened to be asked.
