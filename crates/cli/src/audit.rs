...
/// - [`Identity::Owner`] is what the password and the bearer token both produce,
///   and they are not distinguishable from here. It is recorded as a
///   password-opened session, which is what a console login is; a bearer-driven
///   session would be recorded one notch stronger than it was.
///
/// The fix is one field on `Handover` in `crates/admin`, and it is recorded as a
/// follow-up rather than papered over: an inference that is written down is
/// something a reader can allow for, and one that is not is a fact they will
/// believe.
