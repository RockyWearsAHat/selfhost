...
//!
//! - [`identity`] — **who**. [`Identity`] is `Owner` or `Person(PersonName)`,
//!   and `PersonName` is a validated newtype. This is where the magic string
//!   `"owner"` stops being a `const` duplicated across crates and starts being
//!   a variant that a name cannot be spelled into.
...
//!
//! # What it decides today, and why it is here anyway
//!
//! Today every real caller is the owner, and [`Policy::decide`] returns
//! [`Decision::Allow`] for an owner and every capability that exists — with one
//! exception, described on [`Policy`], where an unattended bearer token is
//! refused the two capabilities that drive a machine. So wiring the existing
//! routes onto capability checks changes no observable behaviour at all. That is
//! deliberate and it is the reason this crate ships before there is anything
//! interesting to authorise: a permission model retrofitted onto a route that
//! has already shipped is a model nobody has tested against the route, and the
//! first person to discover the gap is whoever it wrongly lets in.
//!
...
//! //    bearer token is `Identity::Owner` with `Credential::Bearer`; a cookie
