...
//! wiring: the bearer token, the console password, and a password-opened session
//! all mint [`Identity::Owner`](crate::Identity::Owner), and only a passkey — or
//! a session that passkey opened — can mint a person. The policy still refuses
//! the unreachable pairs explicitly rather than assuming them away, because
//! "unreachable" is a property of code in another crate and this crate cannot
//! check it. If per-person tokens or per-person passwords are ever added, they
//! get their own variants here — they must not quietly reuse these, whose whole
//! meaning is "the deployment's own".
...
pub enum Credential {
    ...
    ///
    /// The deployment's root credential and the only one that is not a person
    /// sitting at a browser: the CLI, the native console and the webhook relay
    /// all present it. It is unattended by construction, which is why the
    /// policy refuses it the capabilities that drive the machine unless the
    /// operator arms it in config.
    Bearer,
