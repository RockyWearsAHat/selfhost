...
#
#   scripts/macos/macos-app.sh install     build, bundle, install, pin, and relaunch
#   scripts/macos/macos-app.sh uninstall   unpin, remove the bundle and the CLI link
#   scripts/macos/macos-app.sh bundle      build the .app into target/ and stop there
#
...
install_app() {
  ...
  To remove everything: scripts/macos/macos-app.sh uninstall
DONE
}
...
case "${1:-}" in
  ...
  *)
    cat >&2 <<USAGE
Usage: scripts/macos/macos-app.sh <install|uninstall|bundle>
