...
#
#   scripts/macos/launchd.sh install [daemon-dir] [proxy-dir]
#   scripts/macos/launchd.sh uninstall
#   scripts/macos/launchd.sh status
#
...
status)
    ...
    ;;
*)
    echo "usage: scripts/macos/launchd.sh install [daemon-dir] [proxy-dir] | uninstall | status" >&2
    exit 1
    ;;
esac
